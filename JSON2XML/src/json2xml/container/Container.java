...
public class Container {
	...
	public UrbanDataset getUrbanDataset() {
		return UrbanDataset;
	}
	public void setUrbanDataset(UrbanDataset urbanDataset) {
		this.UrbanDataset = urbanDataset;
	}
	...
	public Specification specification(Node node) {
		...
		//Setting element of Specification class
		if (!eElement.getAttribute("version").isEmpty()) {
			double version = Double.parseDouble(eElement.getAttribute("version"));
			specification.setVersion(version);
		}
		
		IdSpecification id = new IdSpecification();
		id.setValue(eElement.getElementsByTagName("id").item(0).getTextContent());
		...
		if(eElement.getElementsByTagName("properties").item(0)==null) {
			...
			for(int j=1; j< nListProperties.getLength(); j+=2) {
				...
				if (nodeProperty.getNodeType() == Node.ELEMENT_NODE) {
					...
					//Setting element of PropertySpecification class
	...
	public Context context(Node node) {
		...
		//Setting value of Context class
		for(int j=1; j < listContext.getLength();j+=2) {
			...
			if( childContext.getNodeName().equals("producer")) {
				...
				if(elementCoordinate.getElementsByTagName("height").item(0)!=null)
					coordinates.setHeight(Double.parseDouble(elementCoordinate.getElementsByTagName("height").item(0).getTextContent()));
	...
	public Values  values(Node node) {
		...
		ArrayList<PropertyValues> arrayPropertyValues = new ArrayList<PropertyValues>();
		...
		for(int i=0;i<nLines.getLength();i++) {
			...
			if(!eELine.getAttribute("id").isEmpty())
				line.setId(Integer.parseInt(eELine.getAttribute("id")));
